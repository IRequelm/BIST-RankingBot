...
def _style_workbook(path: Path, portfolio_rows: int, current: pd.DataFrame) -> None:
    ...
    below_ma200 = _as_bool(first["bist100_below_ma200"])
    risk_status = "Risk OFF" if below_ma200 else "Risk ON"
    executive_sheet.insert_rows(1, 5)
    executive_sheet["A1"] = "100.000 TL Bugün Nasıl Dağıtılır?"
    executive_sheet["A1"].font = Font(bold=True, size=14, color="1F4E78")
    executive_sheet["A2"] = f"Piyasa Rejimi: {risk_status}"
    executive_sheet["A3"] = f"Güven Puanı: {float(first['confidence_score']):.2f}/100"
    executive_sheet["A4"] = f"Aktif Model: {first['active_model']}"
    executive_sheet["A5"] = "Tablo çekiciliğe göre sıralıdır; yüksek conviction daha güçlü adayları gösterir."
    executive_sheet.freeze_panes = "A7"
    executive_sheet.auto_filter.ref = f"A6:{get_column_letter(executive_sheet.max_column)}{executive_sheet.max_row}"

    portfolio_sheet = workbook["PORTFOY_ONERISI"]
    portfolio_sheet.insert_rows(1, 5)
    portfolio_sheet["A1"] = "Aylık Yatırımcı Özeti"
    portfolio_sheet["A1"].font = Font(bold=True, size=14, color="1F4E78")
    portfolio_sheet["A2"] = f"Toplam önerilen/satılacak satır: {portfolio_rows}"
    portfolio_sheet["A3"] = "Renkler: AL yeşil, TUT sarı, SAT kırmızı."
    portfolio_sheet["A4"] = "Bu rapor araştırma amaçlıdır; otomatik emir veya yatırım tavsiyesi değildir."
    portfolio_sheet.freeze_panes = "A7"
    portfolio_sheet.auto_filter.ref = f"A6:{get_column_letter(portfolio_sheet.max_column)}{portfolio_sheet.max_row}"

    for sheet in workbook.worksheets:
        headers = {cell.value: cell.column for cell in sheet[1]}
        if sheet.title in {"PORTFOY_ONERISI", "YONETICI_OZETI"}:
    ...
    conviction_col = {cell.value: cell.column for cell in executive_sheet[6]}.get("Conviction")
    conviction_fills = {
        "VERY HIGH": "C6E0B4",
        "HIGH": "D9EAD3",
        "MEDIUM": "FFF2CC",
    ...
    if conviction_col:
        for row in range(7, executive_sheet.max_row + 1):
...
def generate_investor_report(stock_prices: dict[str, pd.DataFrame], results_dir: str) -> tuple[Path, Path]:
    ...
    _style_workbook(xlsx_path, len(portfolio), current)
    _write_markdown(md_path, executive_summary, portfolio, top20, action_summary, market_regime, paper_trade)
